...
/* $Header: /usr/src/redhat/SOURCES/InMon_Agent-5.6/RCS/sflow_receiver.C,v 1.15 2006/07/10 18:22:22 root Exp $ */
...
static void putNet32(SFLReceiver *receiver, u_int32_t val);
...
void sfl_receiver_tick(SFLReceiver *receiver, time_t now)
{
  ...
  // check the timeout
  if(receiver->sFlowRcvrTimeout && receiver->sFlowRcvrTimeout != 0xFFFFFFFF) {
...
inline static void putAddress(SFLReceiver *receiver, SFLAddress *addr)
{
  ...
  if(addr->type == 0) {
    ...
    if(addr->type == SFLADDRESSTYPE_IP_V4) put32(receiver, addr->address.ip_v4.s_addr);
    else put128(receiver, addr->address.ip_v6.s6_addr);
  }
}
...
inline static void putGateway(SFLReceiver *receiver, SFLExtended_gateway *gw)
{
  ...
  for(int seg = 0; seg < gw->dst_as_path_segments; seg++) {
...
static int computeFlowSampleSize(SFLReceiver *receiver, SFL_FLOW_SAMPLE_TYPE *fs)
{
  ...
  for(SFLFlow_sample_element *elem = fs->elements; elem != NULL; elem = elem->nxt) {
    fs->num_elements++;
    siz += 8; /* tag, length */
    u_int elemSiz = 0;
    switch(elem->tag) {
...
int sfl_receiver_writeFlowSample(SFLReceiver *receiver, SFL_FLOW_SAMPLE_TYPE *fs)
{
  ...
  if(packedSize > (receiver->sFlowRcvrMaximumDatagramSize / 2)) {
  ...
  for(SFLFlow_sample_element *elem = fs->elements; elem != NULL; elem = elem->nxt) {
    ...
    case SFLFLOW_IPV4:
      ...
      put32(receiver, elem->flowType.ipv4.src_ip.s_addr);
      put32(receiver, elem->flowType.ipv4.dst_ip.s_addr);
    ...
    case SFLFLOW_IPV6:
      ...
      put128(receiver, elem->flowType.ipv6.src_ip.s6_addr);
      put128(receiver, elem->flowType.ipv6.dst_ip.s6_addr);
  ...
  assert(((u_char *)receiver->sampleCollector.datap
	  ...
	  - receiver->sampleCollector.pktlen)  == packedSize);
...
static int computeCountersSampleSize(SFLReceiver *receiver, SFL_COUNTERS_SAMPLE_TYPE *cs)
{
  ...
  for(SFLCounters_sample_element *elem = cs->elements; elem != NULL; elem = elem->nxt) {
    cs->num_elements++;
    siz += 8; /* tag, length */
    u_int elemSiz = 0;
    switch(elem->tag) {
...
{  
  ...
  u_int32_t hdrIdx = (receiver->sFlowRcvrAddress.type == SFLADDRESSTYPE_IP_V6) ? 7 : 4;
  ...
  if(receiver->agent->sendFn) (*receiver->agent->sendFn)(receiver->agent->magic,
    ...
    /* send it myself */
    int result = sendto(receiver->agent->receiverSocket,
			receiver->sampleCollector.data,
			receiver->sampleCollector.pktlen,
			0,
			(sockaddr *)&receiver->receiver,
...
static void resetSampleCollector(SFLReceiver *receiver)
{
  ...
  receiver->sampleCollector.datap = (receiver->sFlowRcvrAddress.type == SFLADDRESSTYPE_IP_V6) ?
