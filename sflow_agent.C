...
/* $Header: /usr/src/redhat/SOURCES/InMon_Agent-5.6/RCS/sflow_agent.C,v 1.10 2006/07/10 18:20:22 root Exp $ */
...
void sfl_agent_release(SFLAgent *agent)
{
  ...
  for(SFLSampler *sm = agent->samplers; sm != NULL; ) {
  ...
  for(SFLPoller *pl = agent->pollers; pl != NULL; ) {
  ...
  for(SFLReceiver *rcv = agent->receivers; rcv != NULL; ) {
...
void sfl_agent_tick(SFLAgent *agent, time_t now)
{
  ...
  for(SFLReceiver *rcv = agent->receivers; rcv != NULL; rcv = rcv->nxt) sfl_receiver_tick(rcv, now);
  /* samplers use ticks to decide when they are sampling too fast */
  for(SFLSampler *sm = agent->samplers; sm != NULL; sm = sm->nxt) sfl_sampler_tick(sm, now);
  /* pollers use ticks to decide when to ask for counters */
  for(SFLPoller *pl = agent->pollers; pl != NULL; pl = pl->nxt) sfl_poller_tick(pl, now);
}
...
int sfl_agent_removeSampler(SFLAgent *agent, SFLDataSource_instance *pdsi)
{
  ...
  for(SFLSampler *prev = NULL, *sm = agent->samplers; sm != NULL; prev = sm, sm = sm->nxt) {
...
{
  /* find it, unlink it and free it */
  for(SFLPoller *prev = NULL, *pl = agent->pollers; pl != NULL; prev = pl, pl = pl->nxt) {
...
SFLSampler *sfl_agent_getSampler(SFLAgent *agent, SFLDataSource_instance *pdsi)
{
  ...
  for(SFLSampler *sm = agent->samplers; sm != NULL; sm = sm->nxt)
...
{
  /* find it and return it */
  for(SFLPoller *pl = agent->pollers; pl != NULL; pl = pl->nxt)
