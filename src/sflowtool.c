...
char ip1[100];
struct flowInfo myflow;
map_flowInfo_t mymap;
double total_bd = 5*1024*1024;

const int N = 0;

static void LA(double* conn_bd, const int n, int* num, char* ip, double bd, double t)
{
	*conn_bd += 0.2*1024*1024*(double)n;
	total_bd -= *conn_bd;
	*num = 0;
	printf("线性调整(增): %s, 数据速率: %lfMb/s, 当前连接带宽: %lfMb/s, 距离上次采样时间: %lfs\n", ip, bd/(1024*1024), *conn_bd/(1024*1024),t);
}
static void LA_decrease(double* conn_bd, const int n, int* num, char* ip, double bd, double t)
{
	*conn_bd -= 0.2*1024*1024*(double)n;
	total_bd += *conn_bd;
	*num = 0;
	printf("线性调整(减): %s, 数据速率: %lfMb/s, 当前连接带宽: %lfMb/s, 距离上次采样时间: %lfs\n", ip, bd/(1024*1024), *conn_bd/(1024*1024),t);
}
static void LA_0(double* conn_bd, int* num, char* ip, double bd, double t)
{
	total_bd += *conn_bd - 0.2*1024*1024;
	*conn_bd = 0.2*1024*1024;
	*num = 0;
	printf("线性调整(减): %s, 数据速率: %lfMb/s, 当前连接带宽: %lfMb/s, 距离上次采样时间: %lfs\n", ip, bd/(1024*1024), *conn_bd/(1024*1024),t);
}
static void writeFlowLine(SFSample *sample)
{
  ...
  strcat(ip1, printAddress(&sample->s.ipsrc, &srcIP));
  strcat(ip1, "->");
  strcat(ip1, printAddress(&sample->s.ipdst, &dstIP));
  ...
  while ((key = map_next(&mymap, &iter))) 
  {
    if (strcmp(key, ip1) == 0)
    {
      ...
      map_get(&mymap, ip1)->IP_src = printAddress(&sample->s.ipsrc, &srcIP);
      map_get(&mymap, ip1)->IP_dst = printAddress(&sample->s.ipdst, &dstIP);
      if(tv.tv_sec == map_get(&mymap, ip1)->time_last.tv_sec){
	map_get(&mymap, ip1)->bandwidth = (double)sample->s.sampledPacketSize * 1000 / ((double)tv.tv_usec/1000000-(double)(map_get(&mymap, ip1)->time_last.tv_usec)/1000000);}
      else{map_get(&mymap, ip1)->bandwidth = (double)sample->s.sampledPacketSize * 1000 /((double)tv.tv_sec - (double)map_get(&mymap, ip1)->time_last.tv_sec);}
      if(map_get(&mymap, ip1)->bandwidth > 10*1024*1024)
      {
      	memset(ip1, 0, sizeof(ip1));
	return;
      }
        // byte/us
      temp = (double)tv.tv_sec + (double)tv.tv_usec/1000000 - (double)map_get(&mymap, ip1)->time_last.tv_sec - (double)(map_get(&mymap, ip1)->time_last.tv_usec/1000000);
      gettimeofday( &(map_get(&mymap, ip1)->time_last), NULL);
      if(map_get(&mymap, ip1)->num >= N) //每采样N次进行一次调整
      {
		if(map_get(&mymap, ip1)->connection_bandwidth_now < map_get(&mymap, ip1)->bandwidth)
		{
			if(map_get(&mymap, ip1)->connection_bandwidth_now < map_get(&mymap, ip1)->threshold) //当前带宽小于拥塞门限-->尝试指数调整
			{
				if(map_get(&mymap, ip1)->connection_bandwidth_now * 2 < map_get(&mymap, ip1)->threshold) //目标值<拥塞门限-->指数调整
				{
					if(map_get(&mymap, ip1)->connection_bandwidth_now * 2 <= total_bd) //指数调整成功
					{
						map_get(&mymap, ip1)->connection_bandwidth_now *= 2;
						total_bd -= map_get(&mymap, ip1)->connection_bandwidth_now;
						map_get(&mymap, ip1)->num = 0;
						printf("指数调整(指): %s, 数据速率: %lfMb/s, 当前连接带宽: %lfMb/s, 采样率:%d, 距离上次采样时间: %fs\n", ip1, map_get(&mymap, ip1)->bandwidth/(1024*1024), map_get(&mymap, ip1)->connection_bandwidth_now/(1024*1024), sample->s.meanSkipCount,temp);
					}
					else //指数调整失败,尝试线性调整
					{
						...
						{
							map_get(&mymap, ip1)->threshold = map_get(&mymap, ip1)->connection_bandwidth_now;
							printf("调整失败,更新门限\n");
						}	
					}					
				}
		...
		else
		{
			...
			{
				LA_0(&map_get(&mymap, ip1)->connection_bandwidth_now, &map_get(&mymap, ip1)->num, ip1, map_get(&mymap, ip1)->bandwidth, temp);
			}
		}
      }
      else //采样次数不足N，计数器num++
      {
	printf("流量监控: %s, 数据速率: %lfMb/s, 当前连接带宽: %lfMb/s, 采样率:%d, 距离上次采样时间: %fs\n", ip1, map_get(&mymap, ip1)->bandwidth/(1024*1024), map_get(&mymap, ip1)->connection_bandwidth_now/(1024*1024), sample->s.meanSkipCount, temp);
	map_get(&mymap, ip1)->num++;
      } 
      map_get(&mymap, ip1)->bandwidth_last = map_get(&mymap, ip1)->bandwidth;
      memset(ip1, 0, sizeof(ip1));
      return;
    }
  }  
  gettimeofday(&tv, NULL);
  myflow.IP_src = printAddress(&sample->s.ipsrc, &srcIP);
  myflow.IP_dst = printAddress(&sample->s.ipdst, &dstIP);
  map_set(&mymap, ip1, myflow);
  map_get(&mymap, ip1)->bandwidth = (double)sample->s.sampledPacketSize * 1000 / ((double)tv.tv_sec + (double)(tv.tv_usec/1000000) - (double)map_get(&mymap, ip1)->time_last.tv_sec - (double)(map_get(&mymap, ip1)->time_last.tv_usec/1000000));
  temp =(double)tv.tv_sec + (double)(tv.tv_usec/1000000) - (double)map_get(&mymap, ip1)->time_last.tv_sec - (double)(map_get(&mymap, ip1)->time_last.tv_usec/1000000);
  map_get(&mymap, ip1)->bandwidth_last = map_get(&mymap, ip1)->bandwidth;
  map_get(&mymap, ip1)->connection_bandwidth_now = 0.2*1024*1024;
  total_bd -= 0.5;
  map_get(&mymap, ip1)->threshold = 2*1024*1024;
  map_get(&mymap, ip1)->num = 0;
  gettimeofday( &(map_get(&mymap, ip1)->time_last), NULL);
  printf("建立连接: %s, 数据速率: %lfMb/s, 当前连接带宽: %lfMb/s, 采样率:%d\n", ip1, map_get(&mymap, ip1)->bandwidth/(1024*1024), map_get(&mymap, ip1)->connection_bandwidth_now/(1024*1024), sample->s.meanSkipCount);
  memset(ip1, 0, sizeof(ip1));
